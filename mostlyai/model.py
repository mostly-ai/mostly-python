...
#   filename:  https://raw.githubusercontent.com/mostly-ai/mostly-app-v2/llb2/mstar/public-api/public-api.yaml?token=GHSAT0AAAAAACNUQM2G6MCJ2RNP5D7CI6KMZOPOR5A
#   timestamp: 2024-02-16T14:18:55+00:00
...
class SyntheticDatasetFormat(Enum):
    csv = "CSV"
    parquet = "PARQUET"
    xslx = "XSLX"


class SyntheticTableSampleSeedData(RootModel[str]):
    root: Annotated[
        str,
        Field(
...
class SyntheticDataset(CustomBaseModel):
    ...
    def download(self, fmt: str):
        if self.client and hasattr(self.client, "download"):
            return self.client.download(synthetic_dataset_id=self.id, fmt=fmt)

    def fetch_df(self):
        if self.client and hasattr(self.client, "fetch_df"):
            return self.client.fetch_df(synthetic_dataset_id=self.id)
